...
for(i in 1:dim(winterprecip)[1]){
  winterprecip[i,]=winterprecip[i,] - mean(winterprecip[i,]) # Use precip - mean(year) as original data
}
```
...
comp = cbind(xlongs,xlats,t(matrix(unlist(rlist),nrow = N,ncol = 81)))
...
p_map = function(i){ # plots a ggplot given the column of comp (means at each node)
  ...
  # high = "#2166AC", midpoint = 1,limits=c(0,15)) +
  # 
...
The El Niño and La Niña patterns observed in the PCA appear again in the SOM analysis. However, the N-S precipitation dicotomy in the Western US depending on the Pacific SST cannot be easily identified.
