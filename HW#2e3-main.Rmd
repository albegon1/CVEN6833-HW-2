...
for(i in 1:4){
  ...
  plot(seq(from=1901,length.out=N),scale(pcs[,i]),type="b",xlab="Year",ylab=paste("PC",i),main=paste("Spatial Component (eigenvector) number",i))
}
```
...
for(i in 1:4){
  plot(seq(from=1901,length.out=N), scale(rotpcs[,i]),type="b",xlab="Year",ylab=paste("PC",i),main=paste("Rotated Spatial Component (eigenvector) number",i))
  if(i==1){
...
for(i in 1:4){
  for(j in 1:4){
    fit=locfit::locfit(westernpcs[,j]~pacificpcs[1:113,i])
