...
```{r}
bat.fm <- spLM(formula=y~Elev, data=xx, coords=coords, priors=priors, tuning=tuning, starting=starting, cov.model = "exponential", n.samples = n.samples, verbose = FALSE, n.report = 1000)
# burn-in samples
...
xx=jan[colnames(jan) %in% c("Long","Lat")] # ind.var for Krigging model
xdem=dem[colnames(dem) %in% c("Long","Lat")]
...
for(i in 1:n.samples2){
  ...
  y2=predict.Krig(ykrig,x=xdem,drop.Z = TRUE) # Predict residual from Krig at DEM locations
  ySE[,i]=predictSE(ykrig,x=xdem,drop.Z = TRUE)                 # Predict SE of Krig at DEM 
  
  y1 = beta[i,1] + beta[i,2]*dem[,3]                              # Predict y from MCMC
  yHBM[,i]=y1+y2                                                  # Add MCMC and residuals
  sprintf("Iteration %s complete",i)
}
```
...
```{r}
load("C:/Users/alexb/Google Drive/CVEN 6833 ADAT/zz Homeworks/HW#2/CVEN6833-HW-2/backupv2.RData")
